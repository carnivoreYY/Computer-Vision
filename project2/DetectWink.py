...
def detectWink(frame, location, ROI, cascade):
    eyes = cascade.detectMultiScale(
        ROI, 1.012, 20, 0|cv2.CASCADE_SCALE_IMAGE, (10,10)) 
    for e in eyes:
        e[0] += location[0]
        e[1] += location[1]
...
def detect(frame, faceCascade, eyesCascade):
    ...
    #gray_frame = cv2.bilateralFilter(gray_frame,10,100,100)

    scaleFactor = 1.009 # range is from 1 to ..
    minNeighbors = 27   # range is from 0 to ..
    flag = 0|cv2.CASCADE_SCALE_IMAGE # either 0 or 0|cv2.CASCADE_SCALE_IMAGE 
    minSize = (80,80) # range is from (0,0) to ..
    ...
    for f in faces:
        ...
        h2 = int(1.1*h/2)
        faceROI = gray_frame[y:y+h2, x:x+w]
